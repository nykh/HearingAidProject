...
void DSP_Init_UART2()
{
	Init_Interrupts_UART2();
	Init_I2C();
	if(!Init_AIC3106(SampleRateSetting)) {
		while(1);	// stall on codec init failure
	}
	Init_McASP0();
	Init_UART2_with_Interrupt(115200);
	EnableInterrupts_UART2();
}
...
//	ISTP = 0x80000000;					// set vector table location shared RAM
	ISTP = 0x11800000;					// set vector table location DSP RAM
	intc->intmux3 = 0x0F0E0D00 | 61; 	// assign McASP0 irq to INT12
	intc->intmux1 = 0x07060500 | 69;    // assign UART2_INT irq to INT4
}
...
void EnableInterrupts_UART2() {
  	IER |= 0x1012;  					// enable EDMA3_CC0_INT1 interrupt (8) and UART2 (4)
  	ICR = 0xffff;       				// clear all pending interrupts
 	CSR |= 1;           				// set GIE
}
...
void Init_UART(UART* pUART, Uint32 baud_rate)
///////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////
{
	...
	pUART->pwremu_mgmt = 0;				// reset UART
	wait(500);

	pUART->dll = (divisor & 0xff);			// baud rate divisor
	pUART->dlh = (divisor >> 8);
	pUART->iir_fcr = 0x0000;				// disable FIFOs
	pUART->ier = 0x0000;					// disable interrupts
	pUART->lcr = 0x0003;					// 8-N-1
	pUART->mcr = 0x0000;					// no flow control, no loopback
	pUART->mdr	= 0;						// use 16x oversampling

	pUART->pwremu_mgmt = 0x6001;    		// tx/rx enabled

	dummy = pUART->rbr_thr;				// flush rx data
}

void Init_UART2_with_Interrupt(Uint32 baud_rate)
///////////////////////////////////////////////////////////////////////
// Purpose:   Configures UART2 to operate at the specified baud rate
//
// Input:     baud_rate - desired baud rate
