...
AddIndexFifo(UART0Rx, FIFOSIZE, char, FIFOSUCCESS, FIFOFAIL)
AddIndexFifo(UART0Tx, FIFOSIZE, char, FIFOSUCCESS, FIFOFAIL)
...
void static copyHardwareToSoftware(void){
  char letter;
  while(((UART0_FR_R&UART_FR_RXFE) == 0) && (UART0RxFifo_Size() < (FIFOSIZE - 1))){
    letter = UART0_DR_R;
    UART0RxFifo_Put(letter);
  }
}
...
void static copySoftwareToHardware(void){
  char letter;
  while(((UART0_FR_R&UART_FR_TXFF) == 0) && (UART0TxFifo_Size() > 0)){
    UART0TxFifo_Get(&letter);
    UART0_DR_R = letter;
  }
}
...
unsigned char UART_InChar(void){
  char letter;
  while(UART0RxFifo_Get(&letter) == FIFOFAIL){};
  return(letter);
}
// output ASCII character to UART
// spin if TxFifo is full
void UART_OutChar(unsigned char data){
  while(UART0TxFifo_Put(data) == FIFOFAIL){};
...
void UART_NewLine(void){
	...
	while(UART0TxFifo_Put(LF) == FIFOFAIL){};
  UART0_IM_R &= ~UART_IM_TXIM;          // disable TX FIFO interrupt
  copySoftwareToHardware();
  UART0_IM_R |= UART_IM_TXIM;           // enable TX FIFO interrupt
}
