...
//	uint8_t hi = color >> 8, lo = color;
	...
	if(x1==x2){/////vertical
		if(y1>y2){
			h = y1-y2;
			ST7735_DrawFastVLine(x1, y2, h, color);
		}else if(y1<y2){
			h = y2-y1;
			ST7735_DrawFastVLine(x1, y1, h, color);
		}else{//y1=y2
		...
		if(x1>x2){
			w = x1-x2;
			ST7735_DrawFastHLine(x2, y1, w, color);
		}else if(x1<x2){
			w=x2-x1;
			ST7735_DrawFastHLine(x1, y1, w, color);
		}else{//x1=x2
			ST7735_DrawPixel(x1, y2, color);
		}
	}else{/////slop
		abx = abs(x1-x2);
		aby = abs(y1-y2);
		if(aby>=abx){//y>x--use aby points
			slope = (double)(x2-x1)/(double)(y2-y1);
			if(y1<y2){
				while(aby--){
					tempy = y1+aby;
					tempx = x1+slope*aby;
					ST7735_DrawPixel(tempx, tempy, color);
				}
			}else{
				while(aby--){
					tempy = y2+aby;
...
//************* ST7735_Clock********************************************
//  Draws one Clocke(circle and numbers) on the ST7735 color LCD
//  Inputs: x,y are the center of the circle
//					r is the radius of the circle
// x is horizontal positions, columns from the left edge
//               must be less than 128
//               0 is on the left, 126 is near the right
// y is vertical positions, rows from the top edge
//               must be less than 160
//               159 is near the wires, 0 is the side opposite the wires
//        color 16-bit color, which can be produced by ST7735_Color565() 
// Output: none
void ST7735_Clock(uint16_t x, uint16_t y, uint16_t r, uint16_t color){
//there should be r points in 1/4 of a circle
	int16_t tempx, tempy;
	int16_t r173 = r*1.73/2, rhalf = r/2;
	int16_t rcount1 = r, rcount2 = r, rcount3 = r, rcount4 = r;
	uint16_t scaleindex = 0;
	uint16_t scalelength = (r/10);
	uint16_t inner = (r - scalelength);
	uint16_t inner173 = inner*1.73/2, innerhalf = inner/2;
	uint16_t sx1,sy1,sx2,sy2;
	//******the process to draw the circle******
	while(rcount1--){//4
		tempx = x + rcount1;
		tempy = y + sqrt((double)(r*r - rcount1*rcount1));
		ST7735_DrawPixel(tempx, tempy , color);
		tempy = y + rcount1;
		tempx = x + sqrt((double)(r*r - rcount1*rcount1));
		ST7735_DrawPixel(tempx, tempy , color);
	}
	while(rcount2--){//1
		tempx = x + rcount2;
		tempy = y - sqrt((double)(r*r - rcount2*rcount2));		
		ST7735_DrawPixel(tempx, tempy , color);
		tempy = y - rcount2;
		tempx = x + sqrt((double)(r*r - rcount2*rcount2));
		ST7735_DrawPixel(tempx, tempy , color);		
	}
	while(rcount3--){//3
		tempx = x - rcount3;
		tempy = y + sqrt((double)(r*r - rcount3*rcount3));		
		ST7735_DrawPixel(tempx, tempy , color);	
		tempy = y + rcount3;
		tempx = x - sqrt((double)(r*r - rcount3*rcount3));
		ST7735_DrawPixel(tempx, tempy , color);				
	}
	while(rcount4--){//2
		tempx = x - rcount4;
		tempy = y - sqrt((double)(r*r - rcount4*rcount4));		
		ST7735_DrawPixel(tempx, tempy , color);		
		tempy = y - rcount4;
		tempx = x - sqrt((double)(r*r - rcount4*rcount4));
		ST7735_DrawPixel(tempx, tempy , color);	
	}
	//******the process to draw the scale inside and numbers outside the clock******
	for(scaleindex=0;scaleindex<12;scaleindex++){
		if((scaleindex==1)||(scaleindex==2)){//1 
			if(scaleindex==1){//1
				tempx = x + rhalf + 3;
				tempy = y - r173 - 7;
				ST7735_DrawChar(tempx, tempy, '1', ST7735_Color565(255, 128, 128), 0, 1);
				sx1 = x + innerhalf;
				sy1 = y - inner173;
				sx2 = x + rhalf;
				sy2 = y - r173;
				ST7735_Line(sx1, sy1, sx2, sy2, ST7735_Color565(255, 128, 128));
			}else{//2
				tempx = x + r173 + 3;
				tempy = y - rhalf - 6;
				ST7735_DrawChar(tempx, tempy, '2', ST7735_Color565(255, 128, 128), 0, 1);
				sx1 = x + inner173;
				sy1 = y - innerhalf;
				sx2 = x + r173;
				sy2 = y - rhalf;
		...
		}else if((scaleindex==4)||(scaleindex==5)){//4 
			if(scaleindex==4){//4
				tempx = x + r173 + 2;
				tempy = y + rhalf;
				ST7735_DrawChar(tempx, tempy, '4', ST7735_Color565(255, 128, 128), 0, 1);
				sx1 = x + inner173;
				sy1 = y + innerhalf;
				sx2 = x + r173;
				sy2 = y + rhalf;
				...
				tempx = x + rhalf;
				tempy = y + r173 + 3;
				ST7735_DrawChar(tempx, tempy, '5', ST7735_Color565(255, 128, 128), 0, 1);
				sx1 = x + innerhalf;
				sy1 = y + inner173;
				sx2 = x + rhalf;
				sy2 = y + r173;
		...
		}else if((scaleindex==7)||(scaleindex==8)){//3
			if(scaleindex==7){//7
				tempx = x - rhalf - 6;
				tempy = y + r173 + 3;
				ST7735_DrawChar(tempx, tempy, '7', ST7735_Color565(255, 128, 128), 0, 1);
				sx1 = x - innerhalf;
				sy1 = y + inner173;
				sx2 = x - rhalf;
				sy2 = y + r173;
				...
				tempx = x - r173 - 7;
				tempy = y + rhalf + 2;
				ST7735_DrawChar(tempx, tempy, '8', ST7735_Color565(255, 128, 128), 0, 1);
				sx1 = x - inner173;
				sy1 = y + innerhalf;
				sx2 = x - r173;
				sy2 = y + rhalf;
		...
		}else if((scaleindex==10)||(scaleindex==11)){//2
			if(scaleindex==10){//10
				tempx = x - r173 - 13;
				tempy = y - rhalf - 6;
				ST7735_DrawChar(tempx, tempy, '1', ST7735_Color565(255, 128, 128), 0, 1);
				tempx = x - r173 - 7;
				ST7735_DrawChar(tempx, tempy, '0', ST7735_Color565(255, 128, 128), 0, 1);
				sx1 = x - inner173;
				sy1 = y - innerhalf;
				sx2 = x - r173;
				sy2 = y - rhalf;
		...
		}else if((scaleindex==0)||(scaleindex==6)){//---------V
			if(scaleindex==0){//0
				tempy = y - r - 10;
				tempx = x - 6;
				ST7735_DrawChar(tempx, tempy, '1', ST7735_Color565(255, 128, 128), 0, 1);				
				...
				tempy = y + r + 3;
				tempx = x-3;
				ST7735_DrawChar(tempx, tempy, '6', ST7735_Color565(255, 128, 128), 0, 1);
				sx1 = x;
				sy1 = y + inner;
				sx2 = x;
				sy2 = y + r;
			...
			if(scaleindex==3){//3
				tempx = x + r + 3;
				tempy = y - 3;
				ST7735_DrawChar(tempx, tempy, '3', ST7735_Color565(255, 128, 128), 0, 1);
				sx1 = x + inner;
				sy1 = y;
				sx2 = x + r;
				sy2 = y;
				...
				tempx = x - r - 8;
				tempy = y - 3;
				ST7735_DrawChar(tempx, tempy, '9', ST7735_Color565(255, 128, 128), 0, 1);
				sx1 = x - inner;
				sy1 = y;
				sx2 = x - r;
				sy2 = y;
