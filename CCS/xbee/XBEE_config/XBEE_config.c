...
void XBEE_config(const char *myaddr, const char *destination) {
	// XBee Initialization
	char destination_low[10] = "DL00";
	char my_address[10] = "MY00";

	strcpy(&my_address[2], myaddr);
	strcpy(&destination_low[2], destination);

	XBEE_enter_command_mode();
	XBEE_command(destination_low); // DLxx
	XBEE_command("DH0");
	XBEE_command(my_address);      // MYxx
	XBEE_command("BD7");           // use 115200 baud rate
	XBEE_command("AP0");           // transparent mode
	XBEE_command("WR");
	XBEE_command("CN");
}
